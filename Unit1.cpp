...
void __fastcall TForm1::CreateButtonClick(TObject *Sender)
{
	...
	bool check = true;
	if(StrToInt(MaxCountEdit->Text) < 1)
	{
		MaxCountEdit->Text = "";
		check = false;
	}
	if (StrToInt(HashTableSizeEdit->Text) < 1)
	{
		HashTableSizeEdit->Text = "";
		check = false;
	}
	if (check == false)
	{
        Application->Title = "ОШИБКА";
		ShowMessage("Неверное заполнение полей! \nРазмер хеш-таблицы > 0."
			" Max кол-во злементов > 0");
            return;
	}
	ElementsMemo->Clear();
	ShowMemo->Clear();
...
// переход в соседний Edit при нажатии Enter
